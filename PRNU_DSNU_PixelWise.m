...
% AVERAGE ROUTINE
 ...
 for i = 1:5:length(data_55T.exposureTime)
     ...
     data_55S_avg(j).img = (x1.img + x2.img + x3.img + x4.img  + x5.img)/5;     
 ...
 for i = 1:5:length(data_99T.exposureTime)
     ...
     data_99S_avg(j).img = (x1.img + x2.img + x3.img + x4.img  + x5.img)/5;     
 ...
 for i = 1:5:length(data_198T.exposureTime)
     ...
     data_198S_avg(j).img = (x1.img + x2.img + x3.img + x4.img  + x5.img)/5;     
 ...
 for i = 1:5:length(data_299T.exposureTime)
     ...
     data_299S_avg(j).img = (x1.img + x2.img + x3.img + x4.img  + x5.img)/5;     
 ...
 for i = 1:5:length(data_395T.exposureTime)
     ...
     data_395S_avg(j).img = (x1.img + x2.img + x3.img + x4.img  + x5.img)/5;     
 ...
 for i = 1:5:length(data_798T.exposureTime)
     ...
     data_798S_avg(j).img = (x1.img + x2.img + x3.img + x4.img  + x5.img)/5;     
...
data_798T_avg = sortrows(T, 'exposureTime'); % sort the table by 'exposureTime'
 ...
 for y=1:length(data_55S_avg(1).img(1,:))
     for x=1:length(data_55S_avg(1).img(:,1))
         temp = double([data_55S_avg(1).img(x,y); data_55S_avg(2).img(x,y); data_55S_avg(3).img(x,y); data_55S_avg(4).img(x,y); data_55S_avg(5).img(x,y)]);
         ...
         temp = double([data_198S_avg(1).img(x,y); data_198S_avg(2).img(x,y); data_198S_avg(3).img(x,y); data_198S_avg(4).img(x,y); data_198S_avg(5).img(x,y)]);
         result = polyfit(data_198T_avg.exposureTime, temp, 1);
         m_198(x,y) = result(1);
         b_198(x,y) = result(2);
         
         temp = double([data_299S_avg(1).img(x,y); data_299S_avg(2).img(x,y); data_299S_avg(3).img(x,y); data_299S_avg(4).img(x,y); data_299S_avg(5).img(x,y)]);
...
filename = 'PRNU_DSNU_PixelWise_Data.mat';
...
figure(4)
imagesc(m_part(1).m)
title('Exposure Slope for Each Pixel (ISO55 - Averaged Frames) : Entire Image')
colorbar

figure(5)
%imagesc(m_part(1).m(1500:2000,1500:2000))
imagesc(m_part(1).m(1:500,1:500))
%title('Exposure Slope for Each Pixel (ISO55 - Averaged Frames) : Local Area (1500:2000, 1500:2000)')
title('Exposure Slope for Each Pixel (ISO55 - Averaged Frames) : Local Area (1:500,1:500)')
colorbar

figure(6)
histogram(m_part(1).m)
title('Exposure Slope for Each Pixel (ISO55 - Averaged Frames) : Entire Image')


figure(7)
%histogram(m_part(1).m(1500:2000,1500:2000))
histogram(m_part(1).m(1:500,1:500))
%title('Exposure Slope for Each Pixel (ISO55 - Averaged Frames) : Local Area (1500:2000, 1500:2000)')
title('Exposure Slope for Each Pixel (ISO55 - Averaged Frames) : Local Area (1:500,1:500)')
...
for i=1:length(m_part)
    ...
    temp = m_part(i).grSLP(1:500,1:500);
    temp_reshape = nonzeros(temp);
    PRNU(i).grSLP = mean2(temp_reshape);
    PRNU(i).grSTDSLP = std2(temp_reshape);
    PRNU(i).grPRNU = std2(temp_reshape)/mean2(temp_reshape)*100;
    %Gb
    %temp = m_part(i).gbSLP(1500:2000,1500:2000);
    temp = m_part(i).gbSLP(1:500,1:500);
    temp_reshape = nonzeros(temp);
    PRNU(i).gbSLP = mean2(temp_reshape);
    PRNU(i).gbSTDSLP = std2(temp_reshape);
    PRNU(i).gbPRNU = std2(temp_reshape)/mean2(temp_reshape)*100;
    %R
    %temp = m_part(i).rSLP(1500:2000,1500:2000);
    temp = m_part(i).rSLP(1:500,1:500);
    temp_reshape = nonzeros(temp);
    PRNU(i).rSLP = mean2(temp_reshape);
    PRNU(i).rSTDSLP = std2(temp_reshape);
    PRNU(i).rPRNU = std2(temp_reshape)/mean2(temp_reshape)*100;
    %B
    %temp = m_part(i).bSLP(1500:2000,1500:2000);
    temp = m_part(i).bSLP(1:500,1:500);
    temp_reshape = nonzeros(temp);
    PRNU(i).bSLP = mean2(temp_reshape);
    PRNU(i).bSTDSLP = std2(temp_reshape);
    PRNU(i).bPRNU = std2(temp_reshape)/mean2(temp_reshape)*100;
end
%%
PRNU_T = struct2table(PRNU); % convert the struct array to a table
hold on
plot([55,99,198,299,395,798],PRNU_T.grPRNU,  'DisplayName', 'Gr')
plot([55,99,198,299,395,798],PRNU_T.gbPRNU,  'DisplayName', 'Gb')
plot([55,99,198,299,395,798],PRNU_T.rPRNU,  'DisplayName', 'R')
plot([55,99,198,299,395,798],PRNU_T.bPRNU,  'DisplayName', 'B')
legend
xlabel("ISOSpeed")
ylabel("PRNU (%)")
title("PRNU (%) vs. ISOSpeed")

%%
figure(4)
imagesc(m_part(1).b)
title('Offset for Each Pixel (ISO55 - Averaged Frames) : Entire Image')
colorbar

figure(5)
%imagesc(m_part(1).b(1500:2000,1500:2000))
imagesc(m_part(1).b(1:500,1:500))
%title('Offset for Each Pixel (ISO55 - Averaged Frames) : Local Area (1500:2000, 1500:2000)')
title('Offset for Each Pixel (ISO55 - Averaged Frames) : Local Area (1:500, 1:500)')
colorbar

figure(6)
histogram(m_part(1).b)
title('Offset for Each Pixel (ISO55 - Averaged Frames) : Entire Image')

figure(7)
%histogram(m_part(1).b(1500:2000,1500:2000))
histogram(m_part(1).b(1:500,1:500))
%title('Offset for Each Pixel (ISO55 - Averaged Frames) : Local Area (1500:2000, 1500:2000)')
title('Offset for Each Pixel (ISO55 - Averaged Frames) : Local Area (1:500, 1:500)')

for i=1:length(m_part)
    %Gr
    %temp = m_part(i).grOff(1500:2000,1500:2000);
    temp = m_part(i).grOff(1:500,1:500);
    temp_reshape = nonzeros(temp);
    DSNU(i).grOff = mean2(temp_reshape);
    DSNU(i).grDSNU = std2(temp_reshape);
    %DSNU(i).grDSNU = std2(temp_reshape)/mean2(temp_reshape)*100;
    %Gb
    %temp = m_part(i).gbOff(1500:2000,1500:2000);
    temp = m_part(i).gbOff(1:500,1:500);
    temp_reshape = nonzeros(temp);
    DSNU(i).gbOff = mean2(temp_reshape);
    DSNU(i).gbDSNU = std2(temp_reshape);
    %DSNU(i).gbDSNU = std2(temp_reshape)/mean2(temp_reshape)*100;
    %R
    %temp = m_part(i).rOff(1500:2000,1500:2000);
    temp = m_part(i).rOff(1:500,1:500);
    temp_reshape = nonzeros(temp);
    DSNU(i).rOff = mean2(temp_reshape);
    DSNU(i).rDSNU = std2(temp_reshape);
    %DSNU(i).rDSNU = std2(temp_reshape)/mean2(temp_reshape)*100;
    %B
    %temp = m_part(i).bOff(1500:2000,1500:2000);
    temp = m_part(i).bOff(1:500,1:500);
    temp_reshape = nonzeros(temp);
    DSNU(i).bOff = mean2(temp_reshape);
    DSNU(i).bDSNU = std2(temp_reshape);
    %DSNU(i).bDSNU = std2(temp_reshape)/mean2(temp_reshape)*100;
end
%%
DSNU_T = struct2table(DSNU); % convert the struct array to a table
hold on
plot([55,99,198,299,395,798],DSNU_T.grDSNU,  'DisplayName', 'Gr')
plot([55,99,198,299,395,798],DSNU_T.gbDSNU,  'DisplayName', 'Gb')
plot([55,99,198,299,395,798],DSNU_T.rDSNU,  'DisplayName', 'R')
plot([55,99,198,299,395,798],DSNU_T.bDSNU,  'DisplayName', 'B')
legend
xlabel("ISOSpeed")
ylabel("DSNU (%)")
title("DSNU (%) vs. ISOSpeed")