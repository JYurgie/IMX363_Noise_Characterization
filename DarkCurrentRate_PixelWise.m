...
data_798T_avg = sortrows(T, 'exposureTime'); % sort the table by 'exposureTime'
 ...
 for y=1:length(data_55S_avg(1).img(1,:))
     for x=1:length(data_55S_avg(1).img(:,1))
         ...
         temp = double([data_395S_avg(1).img(x,y); data_395S_avg(2).img(x,y); data_395S_avg(3).img(x,y); data_395S_avg(4).img(x,y); data_395S_avg(5).img(x,y)]);
         result = polyfit(data_395T_avg.exposureTime, temp, 1);
         m_395(x,y) = result(1);
         b_395(x,y) = result(2);
         
         temp = double([data_798S_avg(1).img(x,y); data_798S_avg(2).img(x,y); data_798S_avg(3).img(x,y); data_798S_avg(4).img(x,y); data_798S_avg(5).img(x,y)]);
...
filename = 'DarkCurrentRate_PixelWise_Data.mat';
...
% 
% figure(5)
% imagesc(m_55(1500:2000,1500:2000))
% colorbar
% 
% figure(6)
% imagesc(b_55)
...
for i=1:length(m_part)
    ...
    DCR(i).grDCR = mean2(temp_reshape);
    %Gb
    %temp = m_part(i).gbSLP(1500:2000,1500:2000);
    temp = m_part(i).gbSLP(1:500, 1:500);
    temp_reshape = nonzeros(temp);
    DCR(i).gbDCR = mean2(temp_reshape);
    %R
    %temp = m_part(i).rSLP(1500:2000,1500:2000);
    temp = m_part(i).rSLP(1:500, 1:500);
    temp_reshape = nonzeros(temp);
    DCR(i).rDCR = mean2(temp_reshape);
    %B
    %temp = m_part(i).bSLP(1500:2000,1500:2000);
    temp = m_part(i).bSLP(1:500, 1:500);
    temp_reshape = nonzeros(temp);
    DCR(i).bDCR = mean2(temp_reshape);
end
%%
figure(7)
DCR_T = struct2table(DCR); % convert the struct array to a table
hold on
plot([55,99,198,299,395,798],DCR_T.grDCR,'DisplayName', 'Gr')
plot([55,99,198,299,395,798],DCR_T.gbDCR,'DisplayName', 'Gb')
plot([55,99,198,299,395,798],DCR_T.rDCR,'DisplayName', 'R')
plot([55,99,198,299,395,798],DCR_T.bDCR,'DisplayName', 'B')
%scatter([55,99,198,299,395,798],DCR_T.grDCR,'DisplayName', 'Gr')
%scatter([55,99,198,299,395,798],DCR_T.gbDCR,'DisplayName', 'Gb')
%scatter([55,99,198,299,395,798],DCR_T.rDCR,'DisplayName', 'R')
%scatter([55,99,198,299,395,798],DCR_T.bDCR,'DisplayName', 'B')
legend
xlabel("ISOSpeed")
ylabel("DCR (DN/s)")
title("DCR (DN/s) vs. ISOSpeed")
hold off
